...
- **原因**: ProtocolLoggingStreamクラスで`protocol_logger`が`None`の場合のエラーハンドリングが不完全で、非同期ストリームのラップに問題があった
- **解決策**: 
  - `protocol_logger`が`None`の場合の安全なチェックを追加
  - 非同期ストリーム操作全体をtry-catchでラップ
  - 元のストリームのエラーを適切に伝播するように修正
- **結果**: 修正済み、テストは正常にパス
